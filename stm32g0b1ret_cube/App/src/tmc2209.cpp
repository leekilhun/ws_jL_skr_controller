...
#ifndef DRIVER_TMC2209_CPP_
#define DRIVER_TMC2209_CPP_
  ...
  uint8_t datagram_size)
{
  ...
  {
    delay(ECHO_DELAY_INC_MICROSECONDS);
...
uint32_t TMC2209::read(uint8_t register_address)
{
  ...
  {
  	delay(REPLY_DELAY_INC_MICROSECONDS);
  ...
  {
  	delay(POST_READ_DELAY_INC_MICROSECONDS);
  }
...
uint8_t TMC2209::percentToCurrentSetting(uint8_t percent)
{
  uint8_t constrained_percent = constrain(percent,
    PERCENT_MIN,
    PERCENT_MAX);
  uint8_t current_setting = CMAP(constrained_percent,
    PERCENT_MIN,
    PERCENT_MAX,
    CURRENT_SETTING_MIN,
    CURRENT_SETTING_MAX);
...
{
  uint8_t percent = CMAP(current_setting,
    CURRENT_SETTING_MIN,
    CURRENT_SETTING_MAX,
    PERCENT_MIN,
    PERCENT_MAX);
...
uint8_t TMC2209::percentToHoldDelaySetting(uint8_t percent)
{
  uint8_t constrained_percent = constrain(percent,
    PERCENT_MIN,
    PERCENT_MAX);
  uint8_t hold_delay_setting = CMAP(constrained_percent,
    PERCENT_MIN,
    PERCENT_MAX,
    HOLD_DELAY_MIN,
    HOLD_DELAY_MAX);
...
{
  uint8_t percent = CMAP(hold_delay_setting,
    HOLD_DELAY_MIN,
    HOLD_DELAY_MAX,
    PERCENT_MIN,
    PERCENT_MAX);
